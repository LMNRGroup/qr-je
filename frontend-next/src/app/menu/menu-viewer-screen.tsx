...
import { useIdRandomFromPath } from '@/lib/spa-route';
import {
  Facebook,
  Globe,
  Instagram,
...
const MenuViewer = () => {
  const { id, random } = useIdRandomFromPath();
  ...
  const [direction, setDirection] = useState<'left' | 'right' | 0>(0);
    ...
    if (isTwoPageFlip && !isPdf) {
      ...
      if (deltaX < 0) {
        ...
        if (isPdf) {
          ...
          setCurrentPage((prev) => {
            prevPageRef.current = prev;
            return Math.min(menuFiles.length - 1, prev + 1);
          });
        }
      } else {
