...
export function DesktopStudioWizard({
  ...
  const [currentStep, setCurrentStep] = useState<QRWizardStep>(() =>
    selectedQuickAction ? 2 : 1
  );
  ...
  const canProceed = canProceedFromStep(currentStep, wizardState);
  // Allow going back from any step > 1 (can always go back to previous step)
  const canGoBack = currentStep > 1;
  const nextStep = getNextStep(currentStep, wizardState);
