...
interface ConfigPageProps {
  ...
  setProfileForm: (fn: (prev: ProfileFormState) => ProfileFormState) => void;
  initialProfileForm: ProfileFormState | null;
  ...
  avatarOptions: ReadonlyArray<{ id: string; label: string; Icon?: typeof UserIcon }>;
  avatarColors: ReadonlyArray<{ id: string; label: string; bg: string; text: string }>;
}
...
export function ConfigPage({
  ...
  const avatarLetter =
    (profileForm.fullName || user?.email || 'Q').trim().charAt(0).toUpperCase() || 'Q';
            ...
            <div className="glass-panel rounded-2xl p-6 text-sm text-muted-foreground space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {isUsernameCooldown &&
                      t('Username changes are on cooldown.', 'El cambio de usuario esta en espera.')}
                    {usernameStatus === 'checking' &&
                    ...
                    {!isUsernameCooldown &&
                      usernameStatus === 'idle' &&
                      t('Usernames can be changed once every 30 days.', 'Los nombres de usuario se pueden cambiar cada 30 dias.')}
                  </div>
