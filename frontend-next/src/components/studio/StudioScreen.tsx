...
const DesktopStudioWizard = lazy(() => import('@/components/DesktopStudioWizard').then((module) => ({ default: module.DesktopStudioWizard })));
const AdaptiveQRCWizard = lazy(() => import('@/components/AdaptiveQRCWizard').then((module) => ({ default: module.AdaptiveQRCWizard })));
const AdaptiveQRCEditor = lazy(() => import('@/components/AdaptiveQRCEditor').then((module) => ({ default: module.AdaptiveQRCEditor })));
const ArsenalPage = lazy(() => import('./ArsenalPage').then((module) => ({ default: module.ArsenalPage })));
const IntelPage = lazy(() => import('./IntelPage').then((module) => ({ default: module.IntelPage })));
const AdaptivePage = lazy(() => import('./AdaptivePage').then((module) => ({ default: module.AdaptivePage })));
const UpgradePage = lazy(() => import('./UpgradePage').then((module) => ({ default: module.UpgradePage })));
const ConfigPage = lazy(() => import('./ConfigPage').then((module) => ({ default: module.ConfigPage })));
...
const Index = () => {
  ...
  // Preserves the previous `navigate('/path')` call sites on top of next/navigation.
  ...
  const isValidWebsiteUrl = useCallback((value: string) => {
    const trimmed = value.trim();
  ...
  const normalizePhone = useCallback((value: string) => value.replace(/[^\d+]/g, '').trim(), []);
  ...
  const normalizedPhone = useMemo(() => normalizePhone(phoneNumber), [normalizePhone, phoneNumber]);
  ...
  const appBaseUrl = typeof window !== 'undefined'
    ...
    : (process.env.NEXT_PUBLIC_APP_URL ?? 'https://qrcode.luminarapps.com');
  ...
  const parseKind = useCallback((kind?: string | null): { mode: 'static' | 'dynamic'; type: string } => {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(poll, 180000);
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    const fetchSummary = async (showLoading: boolean) => {
      ...
      try {
        ...
        if (!cancelled && showLoading) setIntelLoading(false);
      }
    };
    fetchSummary(true);
    // Reduced from 30s to 120s to significantly reduce egress usage
    const pollTimer = window.setInterval(() => fetchSummary(false), 120000);
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    fetchAreas(true);
    // Reduced from 60s to 180s (3 minutes) to significantly reduce egress usage
    const pollTimer = window.setInterval(() => fetchAreas(false), 180000);
    return () => {
      cancelled = true;
      if (pollTimer) window.clearInterval(pollTimer);
    };
  ...
  const handleGenerate = async (name?: string | null, editBehavior: 'stay' | 'arsenal' = 'arsenal') => {
    ...
    try {
      ...
      if (message.includes('NEXT_PUBLIC_API_BASE_URL')) {
        toast.error('API base URL is missing. Add NEXT_PUBLIC_API_BASE_URL to frontend-next/.env.local.');
      } else {
        toast.error(message);
      }
    } finally {
    ...
    <>
      ...
      {isMobile && (
        <>
          <button
            type="button"
            className={`fixed ${isLeftie ? 'left-6' : 'right-6'} flex items-center justify-center rounded-full border border-amber-300/50 bg-card/80 p-1.5 shadow-lg shadow-[0_0_14px_rgba(251,191,36,0.2)] transition hover:border-amber-300/70 hover:bg-card ${
              isDialOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'
            } ${tourActive && isTourDialStep ? 'z-[95]' : 'z-[70]'}`}
            style={{
              bottom: 'calc(var(--qrc-footer-h, 0px) + max(0.75rem, env(safe-area-inset-bottom)))',
            }}
      ...
      <main
        className={`container mx-auto px-4 py-3 sm:py-6 lg:py-8 transition ${
          showGuestWelcome || isBooting ? 'blur-md pointer-events-none select-none' : ''
        } ${isMobile ? 'flex flex-col gap-3 pb-24' : ''} ${isMobileV2 ? 'qrc-v2-main' : ''}`}
      >
        {activeTab === 'studio' && (
          <>
              ...
              <div className="glass-panel rounded-2xl p-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    { id: 'file', label: 'File', Icon: FileIcon, onClick: handleStartFile },
        ...
        <section id="studio" className={`mt-4 space-y-4 sm:space-y-5 lg:mt-0 lg:pt-0 lg:space-y-8 ${isMobileV2 ? 'qrc-v2-section' : ''}`}>
          ...
          <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-4 sm:gap-5 lg:gap-6">
            ...
            >
              ...
              {!isMobileV2 && (
                <div className={`pt-2 border-t border-border/50 ${isMobile ? 'pr-20' : ''}`}>
                  <p className="text-xs text-muted-foreground">
                    Need help? Check out our{' '}
                    <Link href="/faq" className="text-primary hover:underline font-medium">
                      FAQ
        ...
        {activeTab === 'codes' && (
          <ArsenalPage
            ...
            adaptiveGradientText={adaptiveGradientText}
          />
        )}
        ...
        {activeTab === 'analytics' && (
          <IntelPage
            ...
            intelRange={intelRange}
            setIntelRange={setIntelRange}
            intelSummary={intelSummary}
            intelLoading={intelLoading}
            intelTrends={intelTrends}
            scanAreas={scanAreas}
        ...
        {activeTab === 'upgrade' && (
          <UpgradePage
            ...
            setSelectedPlanComparison={setSelectedPlanComparison}
            adaptiveGradientText={adaptiveGradientText}
          />
        )}
        ...
        {activeTab === 'adaptive' && (
          <AdaptivePage
            ...
            setShowAdaptiveWizard={setShowAdaptiveWizard}
            setShowAdaptiveEditor={setShowAdaptiveEditor}
            setShowNavOverlay={setShowNavOverlay}
          />
        )}
