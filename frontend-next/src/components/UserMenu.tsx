...
export function UserMenu({ trigger, onSignOut }: { trigger?: React.ReactNode; onSignOut?: () => Promise<void> }) {
  ...
  const typedTrigger = trigger && isValidElement(trigger)
    ? (trigger as ReactElement<{ className?: string; children?: React.ReactNode }>)
    : null;
  const triggerNode = typedTrigger
    ? cloneElement(typedTrigger, {
        className: `relative overflow-visible ${typedTrigger.props.className ?? ''}`.trim(),
        children: (
          <>
            {notificationBadge}
            {typedTrigger.props.children}
          </>
        ),
      })
    : defaultTrigger;
